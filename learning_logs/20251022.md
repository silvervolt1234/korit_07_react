...
```
Stack 컴포넌트를 보면 객체 형태로 값을 집어넣었고 반면 TextField는 string을 그대로 넣은 상황. 그리고 MUI 속성들은 문제점이 축약어가 너무 많다는건데 공식문서에서 컴포넌트 속성을 읽어보며 학습 필요

EX) spacing의 경우 Stack 컴포넌트 내에 있는 자식 컴포넌트들끼리의 간격으로 full word고 mt는 margain-top의 축약어

# Frontend 애플리케이션 보호
현재까지 Backend의 SecurityConfig 확인시 전체 HTTP 요청에 대해 전부 permitAll()이 적용된 상태로 CommandLineRunner를 통해 user/user admin/admin을 넣어도 써먹지 못하는 상황. 그래서 Backend 설정을 롤백하고 Frontend상에 JWT 인증을 적용한것을 구현하고 그러기 위해 Login 관련 컴포넌트를 추가하고 user 관련 정보를 CRUD하며 로그아웃 역시 필요

## 백엔드 SecurityConfig 수정(롤백)
```java
...
## 프론트엔드 보호 과정
1. Login 컴포넌트 생성 : Login.tsx 생성 후 초기화
```tsx
...
이후 MUI를 바로 적용해 TextField / Button 등을 이용해 return을 작성해 프로토타입을 빠르게 뽑아내기
...
현재까지의 상황에서 isAuthenticated가 false면 Login 컴포넌트를 띄워야하고 isAuthenticated가 true면 Carlist 컴포넌트가 필요

그러면 이하에서 요구되는 코드는 조건부 렌더링에 해당

현재 상황에서 App 컴포넌트의 하위 컴포넌트로 Login과 Carlist가 있다고 생각가능하나 문제는 App 컴포넌트에 isAuthenticated 상태를 정의한게 아니라는 점. 그러면 ContextAPI를 도입하지 않는 이상 Login 컴포넌트에 정의한 상태를 상위 컴포넌트로 보낼 수 없기에 코드가 틀리거나 도입이 필요

대신에 Carlist를 Login 컴포넌트의 하위로 보내서 해결